---
title: "FirmWorks Files Documentation"
description: "Documentation for FirmWorks Files, the native Salesforce application for storing, tagging, finding and sharing files."
---
<img src="images/firmworksfiles.svg" alt="FirmWorks Files" height="200"/>

# FirmWorks Files

FirmWorks Files is a native Salesforce application designed to make the storing, retrieving and editing of Content faster and easier.

## Why Firmwork's Files

- [Features](features.md)
- [For Salesforce Administrators](for-salesforce-admins.md)
- [For Salesforce Developers](for-salesforce-developers.md)

## Getting Started

- [Quick Start Guide](quick-start.md)
- [Permissions and Licensing](permissions.md)
- [The Life of a File](life-of-a-file.md)

## Using FirmWorks Files

- [Using FileViewer](component-appendix.md)
- [Enhanced Upload](enhanced-upload.md)
- [FirmWorks Notes](notes.md)
- [File Reporting](file-reporting.md)
- [Historical File Clean-up Help](file-cleanup.md)

## Configuring

- [FirmWorks Files Configuration](configuration.md)
- [Component Reference](component-reference.md)
- [Additional Setup: tabs, page layouts and Experience Cloud](advanced-config.md)

## Automating

- [FirmWorks Files and Flows](fileviewer-and-flow.md)
- [Passing Dynamic Field Values into Flow Components](flow-dynamic-values.md)
- [Flow Templates and Invocable Actions](flow-templates-and-actions.md)
- [File Events](file-events.md)

## Reference and Support

- [Release Notes](release-notes.md)
- [Troubleshooting](troubleshooting.md)
- [Tips And Known Issues](known-issues.md)
- [How To Give FirmWorks Support Access To Your Org](support-support.md)

## Visit Us

https://getfirmworks.com

support@getfirmworks.com

[Salesforce AppExchange Listing](https://appexchange.salesforce.com/appxListingDetail?listingId=a0N3u00000PGu9TEAT)
